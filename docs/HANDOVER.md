...
**Not yet committed or deployed.** Changed files: `apps/admin-web/src/app/orders/page.tsx`, `apps/api/src/index.ts`, `packages/shared/src/schemas.ts` (modified), `apps/api/src/routes/products.ts` (new) — all uncommitted on `master` as of this session. Since `packages/shared` changed, remember §12's rule (rebuild it — `npm run build --workspace=packages/shared` — before the API/web pick up the new export, though `turbo dev` does this automatically). To ship: commit + push (`admin-web` auto-deploys via git); `zan-app-api` needs the full manual §28 dance since it changed too (new `products.ts` route, new schema) and is not git-connected.
