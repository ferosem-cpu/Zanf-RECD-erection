...
**Not yet done:** not deployed to production (`zan-app-api` / `admin-web` on Vercel) - this pass only touched local dev. The purchase-order print page's live rendering was verified via direct API response inspection rather than a full browser click-through (a browser-automation tool hiccup blocked navigation at the time) - worth a quick visual pass next session before shipping.
