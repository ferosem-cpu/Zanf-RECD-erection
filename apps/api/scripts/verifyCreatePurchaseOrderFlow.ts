...
async function main() {
  ...
  // Find a real supplier to test against
  const supRes = await fetch(`${BASE}/purchase-orders/suppliers`, { headers });
  const suppliers = await supRes.json();
  console.log("Suppliers:", suppliers.map((s: { name: string }) => s.name));
  if (!Array.isArray(suppliers) || suppliers.length === 0) {
    console.log("No suppliers exist - test needs at least one. Aborting.");
    return;
  }
  ...
  const msgRes = await fetch(`${BASE}/agent/conversations/${conv.id}/messages`, {
    ...
    headers,
    body: JSON.stringify({
      message: `create a purchase order for ${supplierName}: 10 bags of cement at 400 rupees each, 18% GST`,
    }),
  });
